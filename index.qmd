...
This is where you load your catch data. For this example, we are simulating weekly catch data for 20 cells closest to a hypothetical closure area. Here, we're also simulating a bias of higher PSC inside the hypothetical closure.
...
catch_data2 <- merge(stat_areas, catch_data_combined, by = "STAT_AREA", all.x = FALSE)


```

## 5. Explore Bycatch Data
In this example, the highest PSC rates of Species A are in areas 665730, 665700, 655730, 655700. However, these areas have the lowest PSC rates for Species B.
```{r}
#| label: fig-psc-catch
...
catch_data_means <- st_as_sf(catch_data2)
...
ggplot() +
  ...
  tidyterra::geom_spatvector(data = catch_data2[catch_data2$SPECIES == "Species_A", ], 
...
all_psc_closures_dates <- as.data.frame(catch_data2) %>%
...
library(knitr)
library(kableExtra) # Adding kableExtra for better species grouping visualization
